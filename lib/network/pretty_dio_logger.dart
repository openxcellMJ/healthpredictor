...
class PrettyDioLogger extends Interceptor {
  ...
  void _printPrettyMap(
    ...
    var tabsMain = tabs;
    final isRoot = tabsMain == initialTab;
    final initialIndent = _indent(tabsMain);
    tabsMain++;
    ...
    data.keys.toList().asMap().forEach((index, dynamic key) {
      ...
      if (value is Map) {
        if (compact && _canFlattenMap(value)) {
          logPrint('║${_indent(tabsMain)} "$key": $value${!isLast ? ',' : ''}');
        } else {
          logPrint('║${_indent(tabsMain)} "$key": {');
          _printPrettyMap(value, tabs: tabsMain);
        }
      } else if (value is List) {
        if (compact && _canFlattenList(value)) {
          logPrint('║${_indent(tabsMain)} "$key": ${value.toString()}');
        } else {
          logPrint('║${_indent(tabsMain)} "$key": [');
          _printList(value, tabs: tabsMain);
          logPrint('║${_indent(tabsMain)} ]${isLast ? '' : ','}');
        }
      } else {
        final msg = value.toString().replaceAll('\n', '');
        final indent = _indent(tabsMain);
        final linWidth = maxWidth - indent.length;
        if (msg.length + indent.length > linWidth) {
          final lines = (msg.length / linWidth).ceil();
          for (var i = 0; i < lines; ++i) {
            logPrint(
                '║${_indent(tabsMain)} ${msg.substring(i * linWidth, math.min<int>(i * linWidth + linWidth, msg.length))}');
          }
        } else {
          logPrint('║${_indent(tabsMain)} "$key": $msg${!isLast ? ',' : ''}');
        }
      }
    });
